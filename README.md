...
## Status
**Fully live**: `https://ninjacart-mcp-hub-production.up.railway.app`, deployed on Railway (auto-deploys from `main`). Full auth flow (Google sign-in → role check → token → authenticated tool call), `query_packtrack_db`, and `search_packtrack_knowledge` (Voyage AI embeddings, knowledge base ingested) all verified working in production. See the notes vault's `05 - Change Log.md` for full history.
